...
lv_obj_t *laser_ui_apply_panel_style(lv_obj_t *obj, lv_color_t bg, int pad)
{
    ...
    lv_obj_set_style_border_color(obj, UI_COLOR_BORDER_GLOW, LV_PART_MAIN);
...
lv_obj_t *laser_ui_create_tab_button(lv_obj_t *parent, const char *text, bool active)
{
    ...
    lv_color_t bg = active ? UI_COLOR_ACCENT_DIM : UI_COLOR_SURFACE;
    lv_color_t pressed = active ? UI_COLOR_ACCENT : lv_color_hex(0x252A3A);
...
lv_obj_t *laser_ui_create_hud_panel(lv_obj_t *parent, lv_color_t bg, int pad)
{
    ...
    lv_obj_set_style_border_color(panel, UI_COLOR_BORDER_GLOW, LV_PART_MAIN);
    lv_obj_set_style_border_opa(panel, LV_OPA_60, LV_PART_MAIN);
    lv_obj_set_style_outline_width(panel, 1, LV_PART_MAIN);
    lv_obj_set_style_outline_color(panel, UI_COLOR_ACCENT_DIM, LV_PART_MAIN);
    lv_obj_set_style_outline_opa(panel, LV_OPA_20, LV_PART_MAIN);
    laser_ui_add_corner_marks(panel, UI_COLOR_ACCENT_DIM);
    return panel;
}
...
void laser_ui_style_energy_slider(lv_obj_t *slider, lv_color_t color)
{
    ...
    lv_obj_set_style_bg_color(slider, lv_color_hex(0x222A38), LV_PART_MAIN);
    lv_obj_set_style_bg_opa(slider, LV_OPA_COVER, LV_PART_MAIN);
    lv_obj_set_style_radius(slider, 4, LV_PART_MAIN);
    lv_obj_set_style_border_width(slider, 1, LV_PART_MAIN);
    lv_obj_set_style_border_color(slider, UI_COLOR_BORDER, LV_PART_MAIN);
...
void laser_ui_style_status_bar(lv_obj_t *bar, lv_color_t color)
{
    ...
    lv_obj_set_style_bg_color(bar, lv_color_hex(0x202838), LV_PART_MAIN);
...
void laser_ui_add_map_grid(lv_obj_t *parent, lv_color_t color)
{
    ...
    for (int i = 0; i < count; ++i) {
        lv_opa_t opa = (offsets[i] == 0) ? LV_OPA_50 : LV_OPA_20;
...
void laser_ui_add_background_pattern(lv_obj_t *parent)
{
    ...
    for (int x = 40; x < LV_HOR_RES; x += 40) {
        lv_obj_t *line = laser_ui_create_deco_rect(parent, 1, LV_PCT(100),
                                                   UI_COLOR_ACCENT_DIM, LV_OPA_10);
        lv_obj_align(line, LV_ALIGN_TOP_LEFT, x, 0);
        lv_obj_move_background(line);
    }

    for (int y = 28; y < UI_MAIN_H; y += 28) {
        lv_obj_t *line = laser_ui_create_deco_rect(parent, LV_PCT(100), 1,
                                                   UI_COLOR_ACCENT_DIM, LV_OPA_10);
        lv_obj_align(line, LV_ALIGN_TOP_LEFT, 0, y);
        lv_obj_move_background(line);
    }

    lv_obj_t *left_glow = laser_ui_create_deco_rect(parent, 4, LV_PCT(100),
                                                    UI_COLOR_ACCENT, LV_OPA_20);
    lv_obj_align(left_glow, LV_ALIGN_LEFT_MID, UI_NAV_EXPANDED_W - 2, 0);
    lv_obj_move_background(left_glow);
}
...
void laser_ui_add_panel_scanline(lv_obj_t *parent, lv_color_t color)
{
    ...
    lv_obj_t *line = laser_ui_create_deco_rect(parent, LV_PCT(100), 1, color, LV_OPA_30);
...
lv_obj_t *laser_ui_create_buoy(lv_obj_t *parent, const char *icon, int page_index)
{
    ...
    lv_obj_set_style_border_color(buoy, UI_COLOR_BORDER_GLOW, LV_PART_MAIN);
...
void laser_ui_buoy_set_active(lv_obj_t *buoy, bool active)
{
    ...
    if (active) {
        ...
        lv_obj_set_style_border_color(buoy, UI_COLOR_ACCENT_GLOW, LV_PART_MAIN);
        lv_obj_set_style_border_opa(buoy, LV_OPA_COVER, LV_PART_MAIN);
        lv_obj_set_style_outline_color(buoy, UI_COLOR_ACCENT_GLOW, LV_PART_MAIN);
