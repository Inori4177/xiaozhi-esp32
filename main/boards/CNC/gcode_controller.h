...
public:
    ...
    void Initialize(McpServer& mcp_server) {
        mcp_server.AddTool("self.engraving.engrave_text",
            ...
            // [bread-compact-wifi/CNC 整合] 勿恢复 [this]：lambda 未使用成员，[this] 会触发 -Wunused-lambda-capture
            [](const PropertyList& properties) -> ReturnValue {
                ...
                // [bread-compact-wifi/CNC 整合] text 已在上方 std::move 进 ctx，此处必须用 ctx->text，
                // 勿改回 text.c_str()，否则 ack 字符串为空/未定义（use-after-move）。
                cJSON_AddStringToObject(ack, "text", ctx->text.c_str());
