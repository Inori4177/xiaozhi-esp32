...
static bool peer_uart_write_raw(const char *data, size_t len)
{
    ...
    return uart_write_bytes(PEER_UART_NUM, data, len) == static_cast<int>(len);
}
...
bool peer_uart_link_send_line(const char *line)
{
    ...
    if (ok) {
        ...
        ESP_LOGW(TAG, "send_line failed: %s", line);
    }
    return ok;
}
...
bool peer_uart_link_init(void)
{
    ...
    uart_config_t cfg = {};
    cfg.baud_rate = CONFIG_MOTION_PEER_UART_BAUD;
    ...
    if (xTaskCreatePinnedToCore(rx_task, "peer_uart_rx", 4096, nullptr, 5, nullptr, 0) != pdPASS) {
    ...
    ESP_LOGI(TAG, "Peer UART%d TX=%d RX=%d @ %d baud",
             ...
             static_cast<int>(PEER_UART_RX_PIN), CONFIG_MOTION_PEER_UART_BAUD);
    return true;
#endif
}
